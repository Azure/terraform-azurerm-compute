...
output "vm_identity" {
  ...
  value       = zipmap(concat(azurerm_virtual_machine.vm_windows.*.id, azurerm_virtual_machine.vm_linux.*.id), concat(azurerm_virtual_machine.vm_windows.*.identity, azurerm_virtual_machine.vm_linux.*.identity))
}
