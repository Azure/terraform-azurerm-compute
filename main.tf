...
resource "azurerm_virtual_machine" "vm-linux" {
  count                         = (false == contains([var.vm_os_simple, var.vm_os_offer], "WindowsServer")) && var.is_windows_image != "true" && (var.data_disk == "false") ? var.nb_instances : 0
...
resource "azurerm_virtual_machine" "vm-linux-with-datadisk" {
  count                         = (false == contains([var.vm_os_simple, var.vm_os_offer], "WindowsServer")) && var.is_windows_image != "true" && var.data_disk == "true" ? var.nb_instances : 0
  name                          = "${var.vm_hostname}${count.index}"
  location                      = var.location
  resource_group_name           = azurerm_resource_group.vm.name
  availability_set_id           = azurerm_availability_set.vm.id
...
resource "azurerm_virtual_machine" "vm-windows" {
  count                         = var.is_windows_image || contains([var.vm_os_simple, var.vm_os_offer], "WindowsServer") && var.data_disk == "false" ? var.nb_instances : 0
  name                          = "${var.vm_hostname}${count.index}"
  location                      = var.location
  resource_group_name           = azurerm_resource_group.vm.name
  availability_set_id           = azurerm_availability_set.vm.id
...
resource "azurerm_virtual_machine" "vm-windows-with-datadisk" {
  count                         = var.is_windows_image == "true" || contains([var.vm_os_simple, var.vm_os_offer], "WindowsServer") && var.data_disk == "true" ? var.nb_instances : 0
  name                          = "${var.vm_hostname}${count.index}"
  location                      = var.location
  resource_group_name           = azurerm_resource_group.vm.name
  availability_set_id           = azurerm_availability_set.vm.id
